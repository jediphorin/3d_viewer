...
viewer::viewer(QWidget *parent)
  ...
  QButtonGroup *group2 = new QButtonGroup(ui->frame_7);
...
unsigned int viewer::check_sliders() {
  ...
  if (ui->X_rotate->value() != 0) res |= ROTATE_X;

  if (ui->Y_rotate->value() != 0) res |= ROTATE_Y;

  if (ui->Z_rotate->value() != 0) res |= ROTATE_Z;

  if (ui->X_move->value() != 0 || ui->Y_move->value() != 0 ||
      ui->Z_move->value() != 0)
    res |= MOVE;

  if (ui->Zoom->value() != 0) res |= SCALE;
...
void viewer::on_reset_clicked() {
  ...
  ui->GL->clr_back = QColor(50, 40, 80);
  ui->GL->clr_vert = QColor(128, 128, 64);
  ui->GL->clr_line = QColor(95, 220, 255);
...
//  сделать это при условии, что модель загружена
...
void viewer::on_point_size_valueChanged(int arg1) {
  ...
  if (ui->GL->points != 0) ui->GL->update();
}
...
void viewer::on_line_size_edit_valueChanged(int arg1) {
  ...
  if (ui->GL->dottedLine != 0) ui->GL->update();
}
...
void viewer::on_filename_returnPressed() {
  ...
  if (!fileInfo.isAbsolute()) {
    ...
    if (!ui->GL->initModel(fileInfo.absoluteFilePath())) {
      ui->label_2->setText("     Facets: " +
                           QString::number(ui->GL->data.facet_count));
      ui->label->setText("Vertecies: " +
                         QString::number(ui->GL->data.vertex_count));
...
void viewer::setFrontSettings() {
  ui->back_color->setStyleSheet("background-color: " + ui->GL->clr_back.name() +
                                ";");
...
void viewer::on_save_image_clicked() {
  if (saveImg.exec() == QDialog::Accepted) {
    ...
    if (selectedFilter == "BMP Files (*.bmp)") {
      format = "BMP";
      if (!filePath.endsWith(".bmp", Qt::CaseInsensitive)) filePath += ".bmp";

    } else {
      format = "JPEG";
      if (!filePath.endsWith(".jpeg", Qt::CaseInsensitive)) filePath += ".jpeg";
    }
...
void viewer::createSnapshot() {
  ...
  if (gifCount == gifFps * gifLength) {
    ...
    if (saveGif.exec() == QFileDialog::Accepted) {
      ...
      if (!filePath.endsWith(".gif", Qt::CaseInsensitive)) filePath += ".gif";

      createGif(filePath);
    }

    QDir imagesDir(dir.absoluteFilePath(screenDir));
    if (imagesDir.exists()) imagesDir.removeRecursively();
